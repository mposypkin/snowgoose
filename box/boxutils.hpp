...
namespace snowgoose {
    ...
    public:
        ...
        template <class FT> static bool cutL2(int s, const Box<FT> &box, const FT* c, FT rq, Segment<FT>& seg) {
            ...
            for (int i = 0; i < n; i++) {
                if (i != s) {
                    FT di = SGMAX(SGABS(c[i] - a[i]), SGABS(c[i] - b[i]));
                    dq += di * di;
                }
            }
            if (dq >= rq)
        ...
        template <class FT> static bool intersect(const Box<FT> &fbox, const Box<FT> &sbox, Box<FT>& result) {
            int n = fbox.mDim;
            bool rv;
            SG_ASSERT(sbox.mDim == n);
            SG_ASSERT(result.mDim == n);
            for (int i = 0; i < n; i++) {
                result.mA[i] = SGMAX(fbox.mA[i], sbox.mA[i]);
                result.mB[i] = SGMIN(fbox.mB[i], sbox.mB[i]);
